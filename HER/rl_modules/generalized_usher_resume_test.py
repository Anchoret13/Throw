...
class ddpg_agent:
    ...
    def save_normalizer(self, og_norm):
        info = {}
        info["size"] = og_norm.size
        info["eps"] = og_norm.eps
        info["default_clip_range"] = og_norm.default_clip_range
        info["local_sum"] = og_norm.local_sum
        info["local_sumq"] = og_norm.local_sumsq
        info["local_count"] = og_norm.local_count
        info["total_sum"] = og_norm.total_sum
        info["total_sumsq"] = og_norm.total_sumsq
        info["total_count"] = og_norm.total_count
        info["mean"] = og_norm.mean
        info["std"] = og_norm.std
        return info
        

    def load_normalizer(self, info, og_norm):
        og_norm.size = info["size"]
        og_norm.eps = info["eps"]
        og_norm.default_clip_range = info["default_clip_range"]
        og_norm.local_sum = info["local_sum"]
        og_norm.local_sumq = info["local_sumq"]
        og_norm.local_count = info["local_count"]
        og_norm.total_sum = info["total_sum"]
        og_norm.total_sumsq = info["total_sumsq"]
        og_norm.total_count = info["total_count"]
        og_norm.norm_mean = info["mean"]
        og_norm.std = info["std"]

    def save_current(self, epoch):
        # pass
        buffer_info = self.buffer.buffers
        actor_info = self.actor_network.state_dict()
        critic_info = self.critic_network.state_dict()
        # env_info = self.env
        o_norm = self.save_normalizer(self.o_norm)
        g_norm = self.save_normalizer(self.g_norm)
        epoch_info = epoch
        # t_info = self.t
        actor_optimizer = self.actor_optim
        critic_optimizer = self.critic_optim
        actor_target = self.actor_target_network.state_dict()
        critic_target = self.critic_target_network.state_dict()
        info_dict = {
            "buffer": buffer_info,
            "actor": actor_info,
            "critic": critic_info,
        ...
        file_name = "train_info"+str(epoch)+".pkl"
    ...
    def resume(self, epochs, pickle_file_name):
        with open(pickle_file_name, 'rb') as file:
            info_dict = pickle.load(file)
        
        previous_epoch = info_dict['epoch_info']
        self.epoch = previous_epoch
        if previous_epoch > epochs:
            raise ValueError("wrong epoch info")

        # self.t = info_dict["t-info"]
        
        # load previous buffer
        self.buffer = replay_buffer(self.env_params, self.args.buffer_size, self.her_module.sample_her_transitions)
        self.buffer.buffers = info_dict["buffer"]
        # load actor and critic network/target_network
        self.actor_network.load_state_dict(info_dict["actor"])
        self.critic_network.load_state_dict(info_dict["critic"])
        self.actor_target_network.load_state_dict(info_dict["actor_target"])
        self.critic_target_network.load_state_dict(info_dict["critic_target"])
        # load normalizer
        self.load_normalizer(info_dict["o_norm"], self.o_norm)
        self.load_normalizer(info_dict["g_norm"], self.g_norm)
        # load optimizer

    def resume_learning(self):
        flag = 1
        self.resume(epochs=self.args.n_epochs, pickle_file_name=self.args.resume_path)
        flag = 0
        self.learn()
    ...
    def learn(self):
        ...
        for epoch in range(current_epoch, self.args.n_epochs):
            self.t = epoch + 1
